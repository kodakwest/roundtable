      ...
      <div className="min-h-0">
        <div className="mb-3 flex items-center justify-between">
          <button
            type="button"
            onClick={onCancel}
            className="flex items-center gap-2 rounded-lg border border-[var(--border-subtle)] px-3 py-2 text-sm text-[var(--ink-secondary)] transition hover:border-[var(--accent)] hover:text-[var(--accent)]"
          >
            <ArrowLeft size={16} />
            Back
          </button>
          <button
            ...
            onClick={handleParse}
            disabled={isParsing}
            className="flex items-center gap-2 rounded-lg bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-[var(--bg-base)] transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
          >
            <Wand2 size={16} />
        ...
        <textarea
          ...
          className="h-[calc(100vh-170px)] min-h-[420px] w-full resize-none rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-elevated)] px-4 py-3 font-mono text-sm leading-6 text-[var(--ink-primary)] outline-none transition focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent-muted)]"
          placeholder="# Discussion Guide: ..."
        />
      </div>

      <div className="min-h-0 overflow-y-auto rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-panel)] p-4">
        {error && (
        ...
        {!guide ? (
          <div className="flex min-h-[420px] items-center justify-center text-center text-sm text-[var(--ink-tertiary)]">
          ...
          <div className="space-y-5">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border-subtle)] pb-4">
              <div>
                <h2 className="text-lg font-semibold text-[var(--ink-primary)]">Editable Preview</h2>
                {confidence !== null && (
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-semibold uppercase tracking-wide text-[var(--ink-secondary)]">Sections</h3>
              {guide.sections.map((section, sectionIndex) => (
                <div key={`${section.title}-${sectionIndex}`} className="rounded-lg border border-[var(--border-subtle)] p-3">
                  <Field
                    label="Section title"
                    value={section.title}
            ...
            <div className="rounded-lg border border-[var(--border-subtle)] p-3">
              <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-[var(--ink-secondary)]">Landing</h3>
              <Field
...
function Field({ label, value, onChange, multiline }: FieldProps) {
  const className =
    "mt-1 w-full rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-elevated)] px-3 py-2 text-sm text-[var(--ink-primary)] outline-none transition focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent-muted)]";
  ...
  return (
    <label className="mb-3 block text-sm font-medium text-[var(--ink-secondary)]">
      {label}
      {multiline ? (
